...
module tb_alexnet_hybrid_schedule_trace;
  logic clk = 1'b0;
  logic rst;
  ...
  always @(posedge clk) begin
    ...
    if (!rst && command_valid && command_ready) begin
      int mask_bits;
      $fdisplay(trace_file, "%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,1,1,%0d,%0d,%0d,%0d,%0d",
          command_layer_id, command_m_base, command_m_count, command_n_base,
          command_n_count, command_k_offset, command_k_count, command_weight_fill,
          command_weight_release, command_k_count, command_tile_tag,
          command_tile_tag, command_mode_split_n64, command_bank_enable);
  ...
  initial begin
    ...
    trace_file = $fopen("hybrid_schedule.csv", "w");
    if (!trace_file) $fatal(1, "cannot open trace");
    $fwrite(trace_file, "layer,m_base,m_count,n_base,n_count,k_offset,k_count,weight_fill,weight_release,patch_fill,patch_release,patch_k_total,patch_tag,tile_tag,split,bank_enable\n");
    rst = 1'b1;
    start_valid = 1'b0;
    start_tag = 16'h4a80;
    command_ready = 1'b0;
