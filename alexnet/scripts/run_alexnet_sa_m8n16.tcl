...
set out_dir [file join $alexnet_root build sa_m8n16_sim]
...
exec {*}$simulator_env xvlog -sv -d SIMULATION \
    ...
    [file join $alexnet_root tb tb_alexnet_sa_m8n16.sv]
exec {*}$simulator_env xelab tb_alexnet_sa_m8n16 -debug typical \
    -sv_root $cpp_build -sv_lib libalexnet_golden_dpi
exec {*}$simulator_env xsim tb_alexnet_sa_m8n16 -runall \
...
close $log_file
if {![string match "*ALEXNET_SA_M8N16_TEST_PASSED*" $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet M8xN16 simulation failed; see $log_path"
}

puts "ALEXNET_SA_M8N16_DPI_PASS seed=$seed random_tiles=$random_tiles"
