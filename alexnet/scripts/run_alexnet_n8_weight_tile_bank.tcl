...
set out_dir [file join $alexnet_root build n8_weight_tile_bank_sim]
...
exec {*}$simulator_env xvlog -sv -d SIMULATION \
    ...
    [file join $alexnet_root tb tb_alexnet_n8_weight_tile_bank.sv]
exec {*}$simulator_env xelab tb_alexnet_n8_weight_tile_bank -debug typical \
    -sv_root $cpp_build -sv_lib libalexnet_golden_dpi
exec {*}$simulator_env xsim tb_alexnet_n8_weight_tile_bank -runall

set log_path [file join $out_dir xsim.log]
set log_file [open $log_path r]
set log_text [read $log_file]
close $log_file
if {![string match "*ALEXNET_N8_WEIGHT_TILE_BANK_TEST_PASSED*" $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet N8 weight-tile-bank simulation failed; see $log_path"
}
puts "ALEXNET_N8_WEIGHT_TILE_BANK_DPI_PASS"
