...
if {![info exists router_runtime_placement]} { set router_runtime_placement 0 }
set sim_top tb_alexnet_n8_output_router
set sim_name n8_router_sim
if {$router_runtime_placement} {
  set sim_top tb_alexnet_n8_output_router_runtime
  set sim_name n8_router_runtime_sim
}
...
exec {*}$simulator_env xvlog -sv -d SIMULATION \
    ...
    [file join $alexnet_root tb tb_alexnet_n8_output_router.sv]
exec {*}$simulator_env xelab $sim_top -debug typical \
...
close $log_file
if {![string match "*ALEXNET_N8_ROUTER_TEST_PASSED*" $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet N8 router simulation failed; see $log_path"
}
if {$router_runtime_placement && ![string match \
    "*ALEXNET_N8_ROUTER_RUNTIME_PLACEMENT_TEST_PASSED*" $log_text]} {
  error "AlexNet router runtime placement coverage failed; see $log_path"
}
puts "ALEXNET_N8_ROUTER_DPI_PASS"
