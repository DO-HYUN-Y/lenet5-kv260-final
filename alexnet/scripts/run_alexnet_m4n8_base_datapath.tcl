...
set out_dir [file join $alexnet_root build m4n8_base_datapath_sim]
...
exec {*}$simulator_env xvlog -sv -d SIMULATION \
    ...
    [file join $alexnet_root tb tb_alexnet_m4n8_base_datapath.sv]
exec {*}$simulator_env xelab tb_alexnet_m4n8_base_datapath -debug typical \
    -sv_root $cpp_build -sv_lib libalexnet_golden_dpi
exec {*}$simulator_env xsim tb_alexnet_m4n8_base_datapath -runall

set log_path [file join $out_dir xsim.log]
set log_file [open $log_path r]
set log_text [read $log_file]
close $log_file
if {![string match "*ALEXNET_M4N8_BASE_DATAPATH_TEST_PASSED*" $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet M4xN8 base-datapath simulation failed; see $log_path"
}
puts "ALEXNET_M4N8_BASE_DATAPATH_DPI_PASS"
