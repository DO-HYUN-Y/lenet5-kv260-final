...
set expected_dsps [expr {4 * $cols}]
set logical_macs [expr {8 * $cols}]
...
set out_dir [file join $alexnet_root build sa_m8n${cols}_ooc 200mhz]
set report_dir [file join $alexnet_root reports sa_m8n${cols} 200mhz]
file mkdir $out_dir
...
read_verilog -sv $pe_source $base_source
read_xdc $xdc_source
synth_design -top alexnet_sa_m4n8 -part $part -mode out_of_context \
    -generic PHYS_ROWS=4 -generic COLS=$cols

set synth_dsp_count [llength [get_cells -hierarchical -filter {REF_NAME == DSP48E2}]]
if {$synth_dsp_count != $expected_dsps} {
  error "M8xN${cols} SA expected $expected_dsps DSP48E2, got $synth_dsp_count"
}
...
if {$setup_wns < 0.0 || $hold_whs < 0.0} {
  error "M8xN${cols} SA failed 200 MHz timing: WNS=$setup_wns WHS=$hold_whs"
}
...
puts $metadata_file "design=alexnet_sa_m8n${cols}"
puts $metadata_file "logical_shape=M8xN${cols}"
puts $metadata_file "physical_shape=4x${cols}"
...
puts "ALEXNET_SA_M8N${cols}_OOC_PASS frequency_mhz=$frequency_mhz dsp=$synth_dsp_count peak_tops=$peak_tops WNS=$setup_wns WHS=$hold_whs"
