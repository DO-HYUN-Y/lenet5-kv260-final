...
set out_dir [file join $alexnet_root build packed_pe_sim]
...
# Vivado exports its bundled 2020-era C++ runtime. Remove that override while
# invoking the host CMake/GCC toolchain, then preload the compatible host
# runtime when xelab/xsim load the C++17 DPI library.
...
exec {*}$simulator_env xvlog -sv -d SIMULATION \
    ...
    [file join $alexnet_root tb tb_alexnet_packed_pe.sv]
exec {*}$simulator_env xelab tb_alexnet_packed_pe -debug typical \
    -sv_root $cpp_build -sv_lib libalexnet_golden_dpi
exec {*}$simulator_env xsim tb_alexnet_packed_pe -runall \
...
close $log_file
if {![string match "*ALEXNET_PACKED_PE_TEST_PASSED*" $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet packed PE simulation failed; see $log_path"
}

puts "ALEXNET_PACKED_PE_DPI_PASS seed=$seed random_tiles=$random_tiles"
