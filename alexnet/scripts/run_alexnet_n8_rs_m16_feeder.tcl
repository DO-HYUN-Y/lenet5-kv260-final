...
set out_dir [file join $alexnet_root build n8_rs_m16_feeder_sim]
...
exec {*}$simulator_env xelab tb_alexnet_n8_rs_m4_feeder -debug typical \
    -generic_top PHYS_ROWS=8 -generic_top M_GROUP=16 \
    -generic_top PERF_PROFILE=1 \
    -sv_root $cpp_build -sv_lib libalexnet_golden_dpi
exec {*}$simulator_env xsim tb_alexnet_n8_rs_m4_feeder -runall

set log_path [file join $out_dir xsim.log]
set log_file [open $log_path r]
set log_text [read $log_file]
close $log_file
if {![string match "*ALEXNET_M16N64_FEEDER_PROFILE_PASS*" $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet N8 RS M16 feeder simulation failed; see $log_path"
}
puts "ALEXNET_N8_RS_M16_FEEDER_DPI_PASS"
