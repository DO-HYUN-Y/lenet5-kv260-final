...
set out_dir [file join $alexnet_root build \
    m4n8_rs_resident_weight_dual_accum_datapath_sim]
...
exec {*}$simulator_env xvlog -sv -d SIMULATION \
    ...
    [file join $alexnet_root tb \
        tb_alexnet_m4n8_rs_resident_weight_dual_accum_datapath.sv]
exec {*}$simulator_env xelab \
    tb_alexnet_m4n8_rs_resident_weight_dual_accum_datapath \
    -sv_root $cpp_build -sv_lib libalexnet_golden_dpi
exec {*}$simulator_env xsim \
    tb_alexnet_m4n8_rs_resident_weight_dual_accum_datapath -runall
...
close $log_file
if {![string match \
        "*ALEXNET_M4N8_RS_RESIDENT_WEIGHT_DUAL_ACCUM_DATAPATH_TEST_PASSED*" \
        $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet dual resident accumulator simulation failed; see $log_path"
}
puts "ALEXNET_M4N8_RS_RESIDENT_WEIGHT_DUAL_ACCUM_DATAPATH_DPI_PASS"
