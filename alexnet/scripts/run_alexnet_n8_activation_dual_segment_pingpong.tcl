...
set out_dir [file join $alexnet_root build n8_activation_dual_segment_pingpong_sim]
...
exec {*}$simulator_env xvlog -sv -d SIMULATION \
    ...
    [file join $alexnet_root tb \
        tb_alexnet_n8_activation_dual_segment_pingpong.sv]
exec {*}$simulator_env xelab \
    tb_alexnet_n8_activation_dual_segment_pingpong \
    -sv_root $cpp_build -sv_lib libalexnet_golden_dpi
exec {*}$simulator_env xsim \
    tb_alexnet_n8_activation_dual_segment_pingpong -runall
...
close $log_file
if {![string match \
        "*ALEXNET_N8_ACTIVATION_DUAL_SEGMENT_PINGPONG_TEST_PASSED*" \
        $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet N8 dual-segment activation simulation failed; see $log_path"
}
puts "ALEXNET_N8_ACTIVATION_DUAL_SEGMENT_PINGPONG_DPI_PASS"
