...
set out_dir [file join $alexnet_root build m8n126_inplace_pool_service_sim]
file mkdir $out_dir
cd $out_dir
...
exec xvlog -sv -d SIMULATION \
    ...
    [file join $alexnet_root tb \
        tb_alexnet_m8n126_inplace_pool_service.sv]
exec xelab tb_alexnet_m8n126_inplace_pool_service -debug typical
exec xsim tb_alexnet_m8n126_inplace_pool_service -runall
...
close $log_file
if {![string match \
        "*ALEXNET_M8N126_INPLACE_POOL_SERVICE_TEST_PASSED*" $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "M8N126 in-place pool service simulation failed; see $log_path"
}
puts "ALEXNET_M8N126_INPLACE_POOL_SERVICE_SIM_PASS"
