...
set out_dir [file join $alexnet_root build sa_m4n8_sim]
...
close $log_file
if {![string match "*ALEXNET_SA_M4N8_TEST_PASSED*" $log_text] ||
    [string match "*Fatal:*" $log_text] ||
    [string match "*ERROR:*" $log_text]} {
  error "AlexNet M4xN8 simulation failed; see $log_path"
}

puts "ALEXNET_SA_M4N8_DPI_PASS seed=$seed random_tiles=$random_tiles"
