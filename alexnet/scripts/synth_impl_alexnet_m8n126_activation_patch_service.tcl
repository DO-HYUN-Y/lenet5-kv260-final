...
set out_dir [file join $alexnet_root build \
    m8n126_activation_patch_service_ooc 200mhz]
set report_dir [file join $alexnet_root reports \
    m8n126_activation_patch_service 200mhz]
file mkdir $out_dir
...
read_xdc $xdc_source
synth_design -top alexnet_m8n126_activation_patch_service -part $part \
    -mode out_of_context
...
if {$synth_dsp != 0} {
  error "activation patch service must consume zero DSP48E2, got $synth_dsp"
}
...
puts $metadata_file "design=alexnet_m8n126_activation_patch_service"
puts $metadata_file "boundary=n8_tile_major_cache_to_conv2_fc8_k_major_m16"
...
puts $metadata_file "max_cache_bytes=64896"
...
if {$setup_wns < 0.0 || $hold_whs < 0.0 ||
  ...
  error "activation patch service failed implementation: WNS=$setup_wns WHS=$hold_whs route_failures=[llength $route_failures]"
}
puts "ALEXNET_M8N126_ACTIVATION_PATCH_SERVICE_OOC_PASS frequency_mhz=$frequency_mhz BRAM36=$synth_bram36 BRAM18=$synth_bram18 WNS=$setup_wns WHS=$hold_whs"
