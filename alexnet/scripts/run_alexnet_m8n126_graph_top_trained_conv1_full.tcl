...
set out_dir [file join $alexnet_root build m8n126_graph_top_conv1_full_sim]
set vector_root [file join $alexnet_root cpp build-release \
    full_graph_pattern_v1 top_conv1_full]
if {![file isdirectory $vector_root]} {
  error "trained full-Conv1 vectors not found; run the full-graph golden verifier with --output-dir"
}
file mkdir $out_dir
cd $out_dir
...
exec xsim tb_alexnet_m8n126_graph_top_trained_conv1_smoke -runall \
    -testplusarg "VECTOR_ROOT=$vector_root" -testplusarg FULL_CONV1
...
close $log_file
if {![string match \
        "*ALEXNET_M8N126_GRAPH_TOP_TRAINED_CONV1_FULL_PASS*" $log_text] ||
    [string match "*Fatal:*" $log_text]} {
  error "trained full-Conv1 graph-top regression failed; see $log_path"
}
puts "ALEXNET_M8N126_GRAPH_TOP_TRAINED_CONV1_FULL_SIM_PASS"
