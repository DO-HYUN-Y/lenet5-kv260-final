...
if {[get_property PROGRESS [get_runs synth_1]] ne "100%"} {
    error "A completed synth_1 run is required before implementation retry"
}

# The normal 200 MHz pass can miss setup by only a few tens of picoseconds.
# This strategy adds an explicit post-route physical-optimization pass without
# changing the RTL, interfaces, PE, or target clock.
...
set bit_output [file join $output_dir alexnet_m4n8_kv260.bit]
set xsa_output [file join $output_dir alexnet_m4n8_kv260.xsa]
...
puts $summary_file "IMPLEMENTATION_STRATEGY=$implementation_strategy"
...
puts $summary_file "DSP48E2=[llength $dsp_cells]"
...
puts $summary_file "BITSTREAM=$bit_output"
puts $summary_file "XSA=$xsa_output"
close $summary_file
...
if {[llength $dsp_cells] != 24 || [llength $bram_cells] != 50 ||
...
file copy -force $bit_source $bit_output
write_hw_platform -fixed -include_bit -force $xsa_output

puts "ALEXNET_M4N8_KV260_IMPL_RETRY_DONE frequency_mhz=200 strategy=$implementation_strategy"
close_project
exit
