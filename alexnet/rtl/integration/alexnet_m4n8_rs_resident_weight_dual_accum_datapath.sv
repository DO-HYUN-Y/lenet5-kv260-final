...
// Conv2-sized multi-channel-chunk row-stationary convolution with one
// resident N8 weight tile and a two-segment accumulator transaction. Each
// chunk atomically starts the raster feeder and accumulator descriptor,
// replays its resident weight tile
...
module alexnet_m4n8_rs_resident_weight_dual_accum_datapath #(
  ...
  generate if (EXTERNAL_COMPUTE) begin : g_shared
    assign shared_cfg_valid = base_cfg_valid;
    ...
    assign shared_chunk_valid = base_chunk_valid;
    assign base_chunk_ready = shared_chunk_ready;
    assign shared_chunk_word_count = chunk_word_count;
    assign shared_chunk_output_width = chunk_output_width;
    assign shared_chunk_n_lane_mask = configured_n_lane_mask_q;
    assign shared_chunk_context_tag = chunk_accum_context_tag;
    assign shared_chunk_tile_tag_base = chunk_tile_tag_base;
    assign shared_chunk_index = chunk_index;
    assign shared_chunk_first = chunk_first;
    assign shared_chunk_final = chunk_final;
    assign shared_tile_start_valid = base_tile_start_valid;
    assign base_tile_start_ready = shared_tile_start_ready;
    assign shared_tile_m_count = base_tile_m_count;
    assign shared_tile_n_lane_mask = base_tile_n_lane_mask;
    assign shared_tile_tag = base_tile_tag;
    assign shared_issue_valid = base_issue_valid;
    assign base_issue_ready = shared_issue_ready;
    assign shared_issue_last = base_issue_last;
    assign shared_issue_act_lo = base_issue_act_lo;
    assign shared_issue_act_hi = base_issue_act_hi;
    assign shared_issue_weight = base_issue_weight;
    ...
    assign accum_chunk_active = shared_chunk_active;
    assign base_tile_done = shared_tile_done;
    assign base_chunk_done = shared_chunk_done;
    assign transaction_done = shared_transaction_done;
    assign base_datapath_idle = shared_datapath_idle;
    assign accum_bank_state = shared_accum_bank_state;
    assign accum_context_error = shared_accum_context_error;
    assign base_protocol_error = shared_protocol_error;
    assign queued_count = shared_queued_count;
  end else begin : g_local
  alexnet_m4n8_dual_accum_base_datapath #(
